...
Arena is committed to complete transparency. Bet custody, randomness, and payouts are all verifiable on-chain — no part of a round depends on trusting Arena.

| | |
|:--|:--|
| **On-Chain Custody** | Bets sit in a Solana program vault, not a custodial wallet |
...
| **On-Chain Storage** | All round data stored on Solana and publicly accessible |

---

## On-Chain Vaults

Every round has its own **vault** — a Program-Derived Address (PDA) owned by Arena's Solana program.

| | |
|:--|:--|
| **Bets In** | Each bet is a SOL transfer to the vault PDA |
| **Custody** | Funds are owned by the program, not by a private key |
| **Payouts Out** | Winners are paid directly from the vault by the program |
| **Sweep** | Any remainder is swept to the round's treasury for jackpot, referrals, and buyback |

Each settled round on the Provably Fair page links to:

| Link | Description |
|:--|:--|
| **Vault Address** | The PDA that held all bets for the round |
| **Bet Transactions** | Every individual bet deposit |
| **Payout Transactions** | Every winner payout from the vault |
| **Sweep Transaction** | The final transfer of remainder to treasury |
...
| Field | Description |
|:--|:--|
| **Vault Address** | The on-chain PDA that held the round's bets |
...
| Field | Description |
|:--|:--|
| `vaultAddress` | The on-chain PDA that held the round's bets |
...
Each bet transaction is a SOL transfer to the round's vault PDA, accompanied by an **on-chain memo** with your bet details:
...
| **3** | Verify the destination is the round's vault PDA |
| **4** | Check the **Memo** instruction — it contains your bet details embedded in the transaction |
...
| **Vault Activity** | Every deposit and payout for the round's vault |
| **Payout Transactions** | Transaction links for each winner |
| **Buyback Transactions** | Jupiter swap transactions |
| **Burn Transactions** | Token burn transactions |
| **Running Totals** | Cumulative rounds, bets, and tokens burned |
...
### Could Arena drain a vault during a round?

No. The round's vault is a program-owned PDA. Funds can only be moved out by the program, and only after the round is finalized — winners are paid directly by the program from the vault, and the remainder is swept once the round closes.
...
Every successful bet creates a Solana transaction that deposits SOL into the round's vault. If your wallet shows the transaction but no corresponding bet, contact support with your transaction signature. All valid bets must have an on-chain record.
